...
首屏 h1 字号取版心宽度的 4.5% ～ 5.5%（`clamp(2rem, 3.5vw, 3.25rem)`），与正文 h1 保持约 1.3 倍层级，不做海报级 hero；比例来自 lieflat 报告模板。
...
`--chart-hero` 可选，默认全墨：只在标题所指的对象没被几何凸显时，给**一张图里的一个主角元素**（峰值、当前点、改版后）；第二处上 hero 就等于没有主角。
分类色 `data-tone` 只用于真的有多个平级类目时，同页不超过 4 个。

**反色是第二种强调，与 hero 共用预算。** 行内 `.hl` 圈标题或论点里的答案词，暗卡 `.card[data-variant="ink"]`
放一页里唯一的决定性数字；两者都只是墨纸互换，不引入新色，深色主题下自动成立。一页最多一块暗卡，
一个标题最多一处 `.hl`；反色与 `--chart-hero` 同时指向不同对象时，读者会找不到主角。来源：lieflat 报告模板 R05 / R11。数据图的选型、硬规则与图型目录在 `charts.md`。
...
具体选择器和 21 个指纹在 `assets/show-me.css` 的 `Recipe geometry` 段。
...
- `larashero3-dotcom/lieflat-charts`（2026-09-06 读取，提交 `eace082`）：吸收其一致性的来源 —— 两极色板加透明度阶派生、明度即数据、单一落点、无框圆角卡、token 单一正本；图型目录、数据形状决策树、静态优先与动态触发规则进入 `charts.md`，59 张图型按其图型概念独立实现为纯 SVG，用本 skill 的 token、字体与运行时，不含上游代码（原 ECharts/Chart.js 图型全部重写，地图与整页报告模板不收）。2026-09-06 第二轮从其 12 套报告模板吸收中西混排规则（中文标题不用负字距、行高回正），落在 `show-me.css` 的 `:lang(zh)` 规则，两级反色（行内 `.hl`、一页一块暗卡）、大数契约（数字与单位分离，单位进 `<small>`）、报告节律（编号眉标 → 结论句 → 证据 → 来源行，口径侧栏）、版心取行长与最宽证据的较大者，以及 `share-card` 定尺卡配方；不复制 Inter 全家、CDN 依赖与联网字体。
