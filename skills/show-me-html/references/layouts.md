...
证据驱动的页面按「编号眉标 → 结论句标题 → 证据 → 来源行」一节一节往下：眉标写问题或范围（「问题一 · 是什么拖慢了发布」），
标题写答案（「排队等 review，比需求膨胀更拖进度」），证据是图、表或指标带，`.src` 交代来源与口径。
眉标和标题各说各的，重复即口吃（`build.py` 会 WARN）。读者随时要查的口径（样本、时间范围、定义、数据来源）
放 `.method-rail`，用 `.with-rail` 包主栏与侧栏：

```html
<section>
  <div class="with-rail">
    <div>
      <p class="eyebrow">问题一 · 是什么拖慢了一次发布</p>
      <h2>排队等 review，比需求膨胀更拖进度</h2>
      <table class="table">
        …
      </table>
      <p class="src">季度调研 · 100 个团队 · 多选</p>
    </div>
    <aside class="method-rail" data-toc-skip aria-label="口径说明">
      <p class="eyebrow">关于这份调研</p>
      <h3>…</h3>
      <p>…</p>
    </aside>
  </div>
...
侧栏是卡片面色，sticky，900px 以下落到主栏之后；`data-toc-skip` 让它的标题不进目录。
一页只有一个侧栏；口径不足三条时不开侧栏，写进首屏 lede 或结尾。来源：lieflat 报告模板 R01。
...
调研 / 数据类状态页改用「报告节律」（见全局版式约定）：每节眉标提问、标题作答、证据下面跟 `.src`。
