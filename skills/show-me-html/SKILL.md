...
新页验证主题、窄屏、实际交互、打印与导出：用 `--snap` 产出的双主题截图与 export.md 做眼检；小修检查受影响行为，可复用同一页面未改变部分的已有证据。
...
最后一次构建直接带 `--open`（自检通过才会打开，不必为打开再单独跑一遍）；用户要求不打开时去掉该参数。
`share-card` 定尺卡加 `--shot` 出 PNG，把图片路径一并报告。
构建输出含 PolyForm 许可 WARN 时，在交付说明里点名这些图块及其非商用限制，由用户决定是否替换。
