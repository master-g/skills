...
本目录下四个 gallery 页面中的 **40 个渲染块**改写自
...
衍生块：`editorial.html` 的 L1–L20 全部；`basics.html` 的 F1–F12、F14–F17；
`glance.html` 的 G4、G19–G22。

其余 19 块（`glance.html` 的 G3、G5–G18；`basics.html` 的 F13；`big.html` 的 B1–B3）
是按上游图型概念独立编写的 SVG 实现，属本仓库 MIT 部分。
`references/charts.md` 的选型规则与图型目录是对上游规则的转述，图型名沿用上游。

商用本目录的衍生块前需取得上游作者许可，或改用独立实现替换。
