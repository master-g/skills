...
回写时改写「上次会话」（分支、验证命令与实际结果、停在何处）和「下次运行」（接下来的计划）。本次确认的决策追加到「已验证的事实」，走不通的路径及原因追加到「失败尝试」，只在有内容时追加。

追加条目用 `- [YYYY-MM-DD]` 前缀放在小节末尾。文件超过 400 行时，从最旧的「失败尝试」条目开始删；仍超限就把「已验证的事实」中的多条窄事实合并为少数通用事实。
