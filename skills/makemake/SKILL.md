...
把清单交给用户,用 AskUserQuestion 一次问清缺口。

先对照**基线**看清单缺什么 —— 每种技术栈都该有的四条:`build`、`test`、`clean`、`lint`(含 fmt)。缺哪条,问哪条。

再问增量,按项目形态挑相关的问,不要全套照搬:
...
**完成条件**:用户对每个问题都给了答复,最终 target 清单里每条都能追到一处出处 —— 项目文件,或用户在这一轮的答复。
...
3. `make -n` 无法覆盖的(需要网络、需要设备),在交付时明确列出来,标为未验证。
