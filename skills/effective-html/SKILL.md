...
5. **Self-containment is absolute.** No CDN scripts, no web fonts, no external images, no framework. System font stacks, inline SVG, vanilla JS. An `<a href>` linking out is fine; a resource *loaded* from the network is not. The file must render identically offline in ten years.
